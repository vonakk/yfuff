...
public class TradeManager {
    ...
    // Активные приглашения: кому отправлено -> от кого
    private final Map<UUID, UUID> pendingInvites = new HashMap<>();
    // Активные сессии по игроку
    ...
    /** Отмена торговли — возвращает все предметы из инвентаря трейда владельцам */
    ...
    /** Запускает обратный отсчёт после того как оба подтвердили */
    public void startCountdown(TradeSession session) {
        ...
        var task = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (p2 != null) p2.sendMessage("§eОбмен через " + seconds[0] + "...");
                seconds[0]--;
            }
    ...
    private void executeTrade(TradeSession session) {
        ...
        // Проверка наличия денег прямо перед обменом
        ...
        // Собираем предметы перед изменением, чтобы проверить место в инвентаре
        ...
        // Предметы — с защитой от переполнения (остатки падают на землю у игрока)
